...
\subsection{Arithmetic coding}\label{sec:algoritmi}

La presente sezione formalizza, tramite pseudocodice e notazione matematica, l'algoritmo di arithmetic coding discusso nel flusso operativo sopra descritto. Il simbolo $|$ indica la concatenazione di stringhe e $\lfloor\cdot\rceil$ l'arrotondamento all'intero più vicino.

L'\autoref{alg:selezione-token} implementa il meccanismo di \emph{arithmetic coding}, cuore della codifica: dato il contesto corrente, il Transformer produce una distribuzione di probabilità sui $K$ codici del codebook; i token con probabilità inferiore a $1/K$ vengono scartati, le probabilità residue vengono rinormalizzate e quantizzate a $D$ bit di precisione, e il messaggio in ingresso determina quale token selezionare. Il numero di bit consumati è pari alla lunghezza del prefisso binario comune tra gli estremi dell'intervallo cumulativo associato al token scelto.
...
Ciononostante, il miglioramento ottenuto con il fine-tuning ha reso possibile una verifica visiva della qualità delle immagini generate con arithmetic coding steganografico. La \autoref{fig:vqgan-sampling-comparison} e la \autoref{fig:vqgan-sampling-attack} mostrano un confronto tra campionamento casuale e campionamento aritmetico steganografico del Transformer con encoder fine-tuned: a parità di seed, le immagini prodotte appaiono strutturalmente simili e prive di artefatti non presenti anche nelle immagini generate tramite sampling, confermando che il meccanismo di arithmetic coding non introduce degradazioni visive rilevabili. Le immagini con encoder fine-tuned non presentano differenze visive apprezzabili rispetto alle analoghe generate senza fine-tuning (riportate nella sezione successiva, \autoref{fig:vqgan-sampling-nofine}), indicando che l'addestramento aggiuntivo dell'encoder non altera la qualità percettiva dell'output, sebbene ne alteri la distribuzione di probabilità.

\begin{figure}[htbp]
	\centering
	\subfloat[Messaggio ``\texttt{42}'']{\includegraphics[width=0.6\textwidth]{message_42.png}}\hfill
	\subfloat[Messaggio ``\texttt{Pippo pippo}'']{\includegraphics[width=0.6\textwidth]{message_pippopippo.png}}
	\caption[Confronto tra sampling casuale e sampling aritmetico steganografico del Transformer VQ-GAN con encoder fine-tuned]{Confronto tra campionamento casuale (colonna sinistra) e campionamento aritmetico steganografico (colonna destra) del Transformer di VQ-GAN (modello S-FLCKR, encoder fine-tuned) per due diversi messaggi segreti, a parità di seed. L'arithmetic coding guida la selezione dei token senza degradare la qualità visiva.}
	\label{fig:vqgan-sampling-comparison}
\end{figure}

\begin{figure}[htbp]
	\centering
	\subfloat[``\texttt{Attack@Dawn}'' (gruppo 1)]{\includegraphics[width=0.7\textwidth]{message_att1.png}}\hfill
	\subfloat[``\texttt{Attack@Dawn}'' (gruppo 2)]{\includegraphics[width=0.7\textwidth]{message_att2.png}}
	\caption[Confronto tra sampling casuale e sampling aritmetico steganografico per Attack@Dawn con encoder fine-tuned]{Confronto tra campionamento casuale (colonna sinistra) e campionamento aritmetico steganografico (colonna destra) del Transformer di VQ-GAN (modello S-FLCKR, encoder fine-tuned) per il messaggio \texttt{Attack@Dawn}, su due gruppi diversi di seed (sei run totali). L'arithmetic coding guida la selezione dei token senza degradare la qualità visiva.}
	\label{fig:vqgan-sampling-attack}
\end{figure}

\section{Equivalenza del Transformer}
...
La \autoref{fig:vqgan-sampling-nofine} mostra un confronto visivo tra campionamento casuale e campionamento aritmetico steganografico per il messaggio ``\texttt{Find me!}'', utilizzando i pesi \emph{originali} del modello S-FLCKR, senza alcun fine-tuning dell'encoder. Su due gruppi indipendenti di seed, le immagini prodotte con arithmetic coding appaiono strutturalmente simili a quelle ottenute tramite campionamento casuale e prive di artefatti aggiuntivi, confermando che il meccanismo di arithmetic coding non introduce degradazioni visive rilevabili e che l'algoritmo di Meteor è applicabile ai token visivi indipendentemente dall'addestramento dell'encoder.

\begin{figure}[htbp]
	\centering
	\subfloat[Messaggio ``\texttt{Find me!}'' (gruppo~1)]{\includegraphics[width=0.6\textwidth]{no_fine_message_findme1.png}}\hfill
	\subfloat[Messaggio ``\texttt{Find me!}'' (gruppo~2)]{\includegraphics[width=0.6\textwidth]{no_fine_message_findme2.png}}
	\caption[Confronto tra sampling casuale e sampling aritmetico steganografico del Transformer VQ-GAN senza fine-tuning]{Confronto tra campionamento casuale (colonna sinistra) e campionamento aritmetico steganografico (colonna destra) del Transformer di VQ-GAN (modello S-FLCKR, pesi originali senza fine-tuning) per il messaggio \texttt{Find me!}, su due gruppi indipendenti di seed. L'arithmetic coding guida la selezione dei token senza degradare la qualità visiva, confermando l'applicabilità dell'algoritmo di Meteor a livello di token visivi già con i pesi originali.}
	\label{fig:vqgan-sampling-nofine}
\end{figure}
...
\begin{enumerate}
	...
	\item Il destinatario riceve l'immagine, la riconverte in token, e tenta di estrarre il messaggio. La decodifica richiede di ripercorrere la sequenza: per ciascuna posizione $t$, il destinatario deve ricostruire la distribuzione $P(s_t \mid s_{<t})$ usando i token $s_{<t}$ così come appaiono nell'immagine ricevuta.
\end{enumerate}
...
\begin{itemize}
	...
	\item Il BER peggiorava ulteriormente (raggiungendo 80--84\%) quando il mittente tentava di anticipare l'effetto del canale simulando il round-trip in pixel a ogni passo di generazione, ovvero decodificando la griglia parziale, ricodificandola con l'encoder LFQ e condizionandosi sui token risultanti $s_{<t}'$, nel tentativo di allineare preventivamente il proprio contesto a quello del destinatario.
\end{itemize}
...
L'esperimento ha prodotto dei risultati positivi. Con l'impiego di codici di correzione d'errore, Reed-Solomon su GF(256) e ripetizione binaria, è stato possibile recuperare il messaggio con \textbf{100\% di affidabilità} anche dopo il passaggio attraverso il canale PNG. La capacità utile si attestava nell'ordine di \textbf{30--50 byte per immagine 256$\times$256} con strength~9, ovvero 9 piani di bit su 18 utilizzati. Utilizzando immagini reali come carrier, invece di campioni generati dal modello autoregressivo, la capacità scala linearmente con la risoluzione: un'immagine 512$\times$512 ha 4 volte i token di una 256$\times$256, offrendo 4 volte la capacità a parità di strength.
...
\begin{table}[htbp]
	...
	\begin{tabular}{lccc}
		...
		VQ-GAN + Meteor              & Token lossless  & $0\%$        & Completo          \\
