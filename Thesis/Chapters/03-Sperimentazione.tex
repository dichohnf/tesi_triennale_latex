...
La \autoref{fig:vqgan-sampling-comparison} e la \autoref{fig:vqgan-sampling-attack} mostrano un confronto visivo tra le due modalità di generazione per due diversi messaggi segreti: a parità di seed, le immagini prodotte appaiono strutturalmente simili e prive di artefatti non presenti anche nelle immagini generate tramite sampling, confermando che il meccanismo di arithmetic coding non introduce degradazioni visive rilevabili rispetto all'approccio tradizionale.
...
\begin{figure}[htbp]
	...
	\caption[Confronto tra sampling casuale e sampling aritmetico steganografico per Attack@Dawn]{Confronto tra campionamento casuale (colonna sinistra) e campionamento aritmetico steganografico (colonna destra) del Transformer di VQ-GAN (modello S-FLCKR) per il messaggio \texttt{Attack@Dawn}, su due gruppi diversi di seed (sei run totali). L'arithmetic coding guida la selezione dei token senza degradare la qualità visiva.}
