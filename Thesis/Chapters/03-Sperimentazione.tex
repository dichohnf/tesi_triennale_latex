...
Il fine-tuning ha prodotto un miglioramento misurabile: l'errore di ricostruzione dei codici si è ridotto significativamente, con un aumento della percentuale di token invariati dopo il round-trip encoding-decoding-encoding fino al 99.82\%. Tuttavia, anche dopo l'addestramento, una frazione non trascurabile di token continuava a subire fluttuazioni. Operando questa analisi abbiamo notato che in realtà il \emph{flip-rate} precedente non risultava pari al 42\% come stimato inizialmente, ma era più basso, circa il 24\%. Questo perché la VQ-GAN non è un canale binario indipendente per ciascun token: la dipendenza tra i token e la propagazione dell'errore attraverso il decoder convoluzionale produce un effetto ``a cascata'' che amplifica gli errori locali. Oltre a ciò rimane il problema della compressione in PNG a 8-bit per colore la quale introduce una variazione dei pixel dovuta alla compressione e alla quantizzazione che altera la ricodifica dei token.
...
\begin{table}[htbp]
	...
	\begin{tabular}{lccc}
		...
		\midrule
		VQ-GAN + Meteor              & Pixel PNG       & $\sim 42\%$  & Parziale          \\
		VQ-GAN + fine tuning encoder & Pixel PNG       & $>0\%$       & Parziale          \\
...
\begin{enumerate}
	...
	\item L'\emph{embedding diretto nei piani di bit} (QIM) supera il problema della propagazione, ma al costo di essere un watermark rilevabile, non una steganografia indistinguibile, quindi fortemente suscetibile ad algoritmi di steganalisi.
\end{enumerate}
