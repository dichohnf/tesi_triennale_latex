...
Il presente capitolo descrive l'implementazione pratica del sistema steganografico presentato nel \autoref{ch:proposta}. Mentre il capitolo precedente si è concentrato sugli aspetti algoritmici e teorici — in particolare gli \autoref{alg:selezione-token}, \ref{alg:codifica-patch} e \ref{alg:codifica-messaggio} per la codifica, e gli \autoref{alg:decodifica-token} e \ref{alg:decodifica-messaggio} per la decodifica — qui vengono illustrati i dettagli realizzativi: l'architettura software, l'organizzazione del codice in classi Python, e le procedure per la generazione e il salvataggio delle immagini.
...
\begin{description}
	\item[\texttt{vqgan.py}:] interfaccia con il modello VQ-GAN pre-addestrato. Espone le funzioni \texttt{encode\_to\_z()} e \texttt{decode\_to\_img()}, che astraggono rispettivamente la codifica di un'immagine nella corrispondente griglia di indici discreti e la decodifica di una griglia di indici nell'immagine RGB finale.
	
	\item[\texttt{encode\_methods.py}:] implementa la classe \texttt{SteganographyEncoder}, che realizza la fase di encoding descritta nell'\autoref{sec:encoding-algorithm}. Gestisce l'inizializzazione dei tensori, l'iterazione sulle posizioni della griglia, la selezione dei token e la verifica di codifica.
	
	\item[\texttt{decode\_methods.py}:] implementa la classe \texttt{SteganographyDecoder}, che realizza la fase di decoding descritta nell'\autoref{sec:decoding-algorithm}. Ricostruisce le distribuzioni di probabilità percorrendo la griglia e recupera i bit codificati.
\end{description}

\section{Le classi SteganographyEncoder e SteganographyDecoder}
...
\begin{enumerate}
	\item Estrae l'immagine di contesto dal dataset condiviso e la codifica tramite \texttt{encode\_to\_z()} per ottenere la matrice di riferimento $\mathbf{R}$.
	...
	\item Decodifica $\mathbf{B}$ tramite \texttt{decode\_to\_img()} per ottenere l'immagine finale e la salva su disco.
\end{enumerate}

\subsection{SteganographyDecoder}

La classe \texttt{SteganographyDecoder} implementa l'\autoref{alg:decodifica-messaggio}. Il suo metodo \texttt{decode\_message()} percorre la griglia nello stesso ordine raster dell'encoding: per ciascuna posizione $(r,c)$, ricostruisce il contesto $\mathbf{c}$, estrae il token $z^*$ dall'immagine ricevuta tramite \texttt{encode\_to\_z()}, e invoca la decodifica del token (\autoref{alg:decodifica-token}) per recuperare i bit. I bit così ottenuti vengono concatenati e riconvertiti nel messaggio originale.
...
\begin{enumerate}
	...
	\item Decodifica $\mathbf{B}$ in un'immagine parziale tramite \texttt{decode\_to\_img()};
	\item Ricodifica l'immagine parziale tramite \texttt{encode\_to\_z()};
	\item Confronta l'indice ricostruito $\hat{\imath}$ con $i^*$.
\end{enumerate}
Se $\hat{\imath} = i^*$ la codifica è considerata valida; altrimenti si ripete con un nuovo tentativo. Dopo 100 tentativi falliti, la selezione viene forzata per evitare un ciclo infinito, accettando il token corrente anche in caso di discordanza. Questo costo computazionale, come discusso nella \autoref{sec:limitazioni}, rende l'encoding significativamente più oneroso della decodifica.

\subsection{Salvataggio in formato PNG}

Le immagini generate vengono salvate in formato PNG (Portable Network Graphics). Tale scelta non è accidentale: il formato PNG è \emph{lossless} e supporta una profondità di colore di 8 bit per canale (24 bit totali per pixel in RGB), preservando integralmente ogni valore dei tre canali di colore senza perdita di informazione. Ciò è essenziale per la corretta decodifica del messaggio, poiché qualsiasi compressione con perdita — come quella del formato JPEG, che opera una quantizzazione delle componenti di frequenza nello spazio YCbCr — altererebbe i valori dei pixel e, di conseguenza, gli indici discreti ottenuti dalla ricodifica tramite \texttt{encode\_to\_z()}, compromettendo il recupero del messaggio. Il formato PNG garantisce invece che l'immagine generata possa essere salvata, trasmessa e ricaricata senza alcuna degradazione.
...
Il ciclo di verifica della codifica rende il processo di encoding computazionalmente oneroso: ogni token può richiedere fino a 100 cicli di decodifica-ricodifica dell'immagine parziale. Per un'immagine $256 \times 256$ con circa $240$ token da codificare, il numero di operazioni coinvolte può diventare significativo, con un rapporto asimmetrico tra i tempi di encoding e decoding.
