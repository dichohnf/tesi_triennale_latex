...
La nozione di \textbf{perfectly secure steganography} \cite{kaptchuk-2021} definisce le condizioni per cui un sistema steganografico è sicuro: uno stego-object è perfettamente sicuro se la sua distribuzione di probabilità è identica a quella dei cover-object naturali prodotti dal medesimo processo generativo. Formalmente, sia $\mathcal{G}$ un modello generativo che produce campioni dalla distribuzione $P_\mathcal{G}$, e sia $\mathcal{S}$ un sistema steganografico che, dati un messaggio $M$ e una chiave segreta $K$, produce uno stego-object $\mathbf{x}$. Sia $P_\mathcal{S}(\mathbf{x} \mid M)$ la distribuzione di probabilità degli stego-object generati da $\mathcal{S}$ per il messaggio $M$, marginalizzata\footnote{In teoria della probabilità, \emph{marginalizzare} significa integrare (o sommare, nel caso discreto) una distribuzione congiunta rispetto a una variabile per ottenere la distribuzione di una sola variabile. In questo contesto: $P_{\mathcal{S}}(\mathbf{x} \mid M) = \sum_{K} P(K) \, P_{\mathcal{S}}(\mathbf{x} \mid M, K)$, dove la somma è su tutte le possibili chiavi $K$ e $P(K)$ è la probabilità di ciascuna chiave (uniforme sullo spazio delle chiavi).} su tutte le possibili chiavi $K$ (la cui scelta casuale fornisce l'entropia necessaria a garantire l'indistinguibilità). Il sistema è perfettamente sicuro se:
\[
...
Sebbene steganografia e watermarking condividano la tecnica di incorporare informazione all'interno di un oggetto multimediale, e nonostante il watermarking abbia costituito per decenni la principale tecnica steganografica nota in letteratura (si pensi al diffuso impiego dei watermark robusti per occultare informazioni in immagini e segnali audio prima dell'avvento della steganografia statistica moderna), le due discipline si sono progressivamente distinte per l'obiettivo di sicurezza e per il modello di avversario, con conseguenze dirette sui requisiti progettuali e sulle metriche di valutazione. La distinzione netta che oggi diamo per acquisita è essa stessa un prodotto dell'evoluzione del dominio: con la nascita della steganalisi statistica e dei paradigmi generativi come SwE, la steganografia ha rivendicato una propria identità separata, spostando il proprio obiettivo primario dall'occultamento robusto all'indistinguibilità.
...
\begin{enumerate}
	\item \textbf{Codifica steganografica}: dato un messaggio segreto $M$ e una chiave $K$, il sistema genera un testo $T$ (lo \emph{stego-text}) in cui il messaggio è nascosto. La generazione avviene passo-passo: a ciascun passo temporale $t$, il modello produce una distribuzione di probabilità $\mathbf{p}_t = P(\cdot \mid \mathbf{x}_{<t})$ sull'intero vocabolario, \emph{senza accesso al messaggio $M$}: la distribuzione è condizionata unicamente dai token già generati, non dal segreto da occultare. I bit del messaggio determinano la selezione del token $x_t$ esclusivamente tramite un meccanismo di \emph{arithmetic coding} sul prefisso della distribuzione troncata, senza alterare la distribuzione stessa. Formalmente, $P(\cdot \mid \mathbf{x}_{<t}) \neq P(\cdot \mid \mathbf{x}_{<t}, M)$, ed è questa indipendenza a garantire l'indistinguibilità statistica.
...
Il campionamento steganografico dei token in \emph{Meteor} è realizzato mediante \emph{arithmetic coding}, una tecnica di codifica entropica\footnote{Una codifica entropica è una famiglia di codifiche \emph{lossless} che assegna a ciascun simbolo un codice di lunghezza inversamente proporzionale alla sua probabilità, minimizzando il numero medio di bit richiesti per la trasmissione.} su cui l'intero framework si fonda.
...
Il numero di bit consumati, che può variare da zero a $D$, non è fissato a priori ma emerge naturalmente dalla geometria degli intervalli, ed è proporzionale all'entropia di Shannon della distribuzione al passo $t$: in contesti ad alta entropia, dove molti token hanno probabilità comparabili, gli intervalli cumulativi sono più frammentati e i prefissi comuni tendono a essere più lunghi, consentendo di codificare più bit per token; viceversa, in contesti a bassa entropia, dove la distribuzione è concentrata su pochi token, il sistema codifica pochi bit o nessuno.\footnote{Formalmente, l'entropia di Shannon di una distribuzione $P$ è $H(P) = -\sum_{i} p_i \log_2(p_i)$. Meteor dimostra che il throughput atteso è asintoticamente proporzionale a $H(P)$, adattando implicitamente il tasso di codifica all'entropia istantanea del canale \cite{kaptchuk-2021}.} È questa la proprietà che consente a Meteor di operare senza una soglia minima di entropia: il sistema si adatta fluidamente alla variabilità del canale, codificando informazione solo quando la distribuzione lo consente.

La decodifica è simmetrica: data la stessa distribuzione e l'indice del token $i^*$ presente nello stego-text, si determina l'intervallo $[C_{i^*\mhyphen1}, C_{i^*})$ e si estrae il prefisso binario comune.

\subsection{Il canale testuale come canale privo di rumore}\label{sec:canale-testuale}

Meteor, e più in generale qualsiasi schema di SwE applicato al dominio testuale, beneficia di una proprietà che può apparire ovvia ma che è invece decisiva quando si tenta la trasposizione ad altri domini mediali: il testo, nella sua rappresentazione digitale, costituisce un \emph{canale di comunicazione intrinsecamente privo di rumore}. I byte che rappresentano le frasi generate vengono trasmessi in maniera precisa: non subiscono compressione con perdita di informazione, né quantizzazione, né alcun altro tipo di degradazione. Di conseguenza, il token $x_t$ selezionato dal mittente durante la codifica steganografica è \emph{esattamente} il token che il destinatario legge durante la decodifica.

Questa proprietà è il fondamento silenzioso su cui poggia l'intero meccanismo di Meteor. L'arithmetic coding descritto nella sezione precedente presuppone che la sequenza di token su cui il mittente si è condizionato sia la stessa che il destinatario osserverà, condizione che nel dominio testuale è sempre verificata. Nel dominio visivo, al contrario, il passaggio attraverso i pixel introduce inevitabilmente una forma di rumore di canale: la quantizzazione a 8~bit per canale colore, la compressione (anche in formati considerati \textit{lossless} come il PNG, che quantizzano comunque a \texttt{uint8}) e, più in generale, qualsiasi trasformazione dell'immagine alterano i token visivi tra il momento in cui vengono generati e il momento in cui vengono ricodificati dal destinatario. Come si vedrà nel \autoref{ch:sperimentazione}, questa differenza fondamentale, un canale testuale \emph{lossless} contrapposto a un canale visivo inevitabilmente \emph{lossy}, è la ragione ultima per cui Meteor funziona con affidabilità completa nel dominio per cui è stato progettato e al contempo la causa primaria delle difficoltà che si incontrano nel trasporlo alle immagini.
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.85\textwidth]{taming_modelfigure.pdf}
	\caption[Rappresentazione schematica dell'architettura VQ-GAN]{Rappresentazione schematica dell'architettura VQ-GAN: l'encoder convoluzionale riduce l'immagine in una griglia di token discreti tramite quantizzazione vettoriale sul codebook; il Transformer autoregressivo modella la distribuzione dei token; il decoder convoluzionale ricostruisce l'immagine. Fonte: \cite{esser-2020}.}
	\label{fig:vqgan-architecture}
\end{figure}
...
Il secondo stadio è un modello Transformer autoregressivo che apprende la distribuzione dei token visivi sulla griglia. Dato un contesto di $f \times f$ token, il modello prevede la distribuzione di probabilità sul token successivo. Questo stadio opera esclusivamente sugli indici discreti del codebook e non interagisce direttamente con lo spazio immagine.

\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.85\textwidth]{taming_attentionslide.pdf}
	\caption[Meccanismo di attention del Transformer nella VQ-GAN]{Meccanismo di attention del Transformer autoregressivo nella VQ-GAN: la generazione del token successivo è condizionata dai token circostanti nella griglia spaziale, permettendo al modello di catturare dipendenze locali e globali. Fonte:~\cite{esser-2020}.}
	\label{fig:vqgan-attention}
\end{figure}
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.90\textwidth]{open-magvit2_framework.pdf}
	\caption[Architettura e funzionamento di Open-MAGVIT2]{Architettura e funzionamento di Open-MAGVIT2: il framework combina un encoder convoluzionale con Lookup-Free Quantization (LFQ) per produrre token binari senza codebook esplicito, e un Transformer autoregressivo (Net2NetTransformer) per la generazione della sequenza di token. Fonte: \cite{luo-2025}.}
	\label{fig:open-magvit2-framework}
\end{figure}

\emph{Open-MAGVIT2} \cite{luo-2025} è un tokenizer visivo all'avanguardia che introduce la \emph{Lookup-Free Quantization} (\textbf{LFQ}), una tecnica di quantizzazione che elimina la necessità di un codebook esplicito.

\subsubsection{Principio della LFQ}
\label{sec:lfq}

A differenza della VQ-GAN, dove ciascun vettore latente $\hat{\mathbf{z}}^{(i,j)} \in \mathbb{R}^{n_z}$ viene confrontato per similarità con tutti i $K$ vettori del codebook (un'operazione di costo $\mathcal{O}(K \cdot n_z)$ per posizione spaziale), la LFQ quantizza ciascuna dimensione latente in modo indipendente, eliminando del tutto il codebook. Un'immagine viene processata da un encoder convoluzionale che produce, per ciascuna posizione spaziale, un vettore di $d$ scalari latenti $\hat{\mathbf{z}}^{(i,j)} \in \mathbb{R}^d$. Ciascuno scalare viene quantizzato al proprio \textbf{segno} ($+1$ o $\mhyphen1$), producendo un codice binario di $d$ bit per posizione spaziale. Formalmente:
\[
\mathbf{z}_q^{(i,j)}[k] = \mathrm{sign}\Bigl(\hat{\mathbf{z}}^{(i,j)}[k]\Bigr) \in \{\mhyphen1, +1\}, \quad k = 1, \dots, d.
\]

L'insieme dei possibili codici è quindi lo spazio ${\{\mhyphen1, +1\}}^d$, che in Open-MAGVIT2 ha dimensione $d = 18$ bit, per un totale implicito di $2^{18} = 262\,144$ possibili codici. Non essendo necessaria una ricerca nel codebook, la quantizzazione ha costo $\mathcal{O}(d)$ per posizione, da cui il nome ``lookup-free''.
